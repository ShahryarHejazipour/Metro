...
class LinesAdapter(private val linesList: List<Line>): RecyclerView.Adapter<LinesAdapter.LinesVH>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LinesVH {

        val binding:RowLineBinding=DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.row_line,parent,false)
    ...
    override fun onBindViewHolder(holder: LinesVH, position: Int) {
        ...
        when(line.id){
            ...
            "1" -> {holder.binding.relLine.setBackgroundResource(R.color.Red)}
            "2" -> {holder.binding.relLine.setBackgroundResource(R.color.Blue)}
